...
| `GET /api/sessions/{token}` | `200 {"session": Session, "content": Content}` | `404` unknown token, `410` expired or revoked, body `{"detail", "candidate": {"name", "email"}, "expires_at"}` so the page can offer the candidate's contact |
...
`Session` as returned: `{"company", "contact", "variant", "created_at", "expires_at", "answers_submitted": bool, "submitted_at": str | null, "answers": {"<question_id>": "<text>"} | null}`. Answers are editable until the link expires, so the stored answers come back with the session for the page to show and prefill. Never returns tenant, ttl, or revoked.
